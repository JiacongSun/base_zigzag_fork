...
def plot_mem_comparison():
    ...
    for x in range(plot_num):
        ...
        axs[x].set_xticklabels([f"{x // 1024}" for x in mem_size_list])
    # add legend and grid
    for x in range(plot_num):
    ...
    # add label
    for x in range(plot_num):
        axs[x].set_xlabel("Memory size [KB]", fontsize=14)
    ...
    plt.savefig("./iscas/mem_comparison.png", dpi=300, bbox_inches="tight")
